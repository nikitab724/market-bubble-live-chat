...
- `POST /api/x-broadcast`: X extension reports the current live broadcast id, which the server writes to the matching enabled X source so the server-side X chat connector attaches without a manual paste. Before storing, the server resolves the broadcast's owner via `broadcasts/show.json` and rejects the report with 409 when it belongs to a different account than the source handle (prevents a tab navigating between two watched streams from pointing both X sources at one broadcast); unknown owners (X API failure) are accepted. Requires the bridge ingest token when `ADMIN_PASSWORD_HASH` is set.
...
Kick uses an iframe embed. Twitch renders through the interactive player API (`player.twitch.tv/js/embed/v1.js` → `Twitch.Player`) so the page can observe pause/play events and programmatically resume; the bare iframe remains as a fallback when the embed script cannot load. X uses the X widgets script when `conversationId` exists; otherwise the page shows an open-stream placeholder link.
...
- **Twitch's embed enforces an occlusion rule ("style visibility").** Anything painted over the player iframe — verified 2026-06-11 with the floating social pill and the layout-toggle bracket — makes Twitch refuse autoplay (console: "minimum requirements for autoplay were not met: style visibility") and, because the check re-runs on every player resize, pause a playing stream on each layout toggle or window resize. The viewer therefore keeps the iframe rect clear: full layout insets `.stream-player` under a 68px card-background band that the pill/toggle float over, mini hoists both controls above the panel edge, and the mini/full toggle skips the view-transition morph while a provider iframe is mounted (the morph's snapshot overlay covers the player, which Twitch also reads as hidden). Do not reintroduce chrome that overlaps the player. The same enforcement pauses the embed while the tab is hidden or the window is covered ("switching tabs or processes") and leaves it paused; the viewer renders Twitch through the interactive player API and `attachTwitchAutoResume` (`src/viewer-stream.mjs`) replays pauses that happened while hidden once the page is visible again — pauses made while visible are the viewer's own and are never overridden.
