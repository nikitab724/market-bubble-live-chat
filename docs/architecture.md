...
- **Twitch's embed enforces an occlusion rule ("style visibility") — known and deliberately not worked around.** Anything painted over the player iframe (here: the floating social pill and the layout-toggle bracket, verified 2026-06-11) makes Twitch refuse autoplay (console: "minimum requirements for autoplay were not met: style visibility") and, because the check re-runs on every player resize, pause a playing stream on layout toggles and window resizes. Twitch also pauses the player when the tab is hidden or the window is covered, and on macOS process switches it does so without any visibility signal reaching the page first — so no page-side hook can reliably auto-resume it (a visibility-keyed resume shipped 2026-06-11 missed exactly that and was reverted). A full workaround (clearing the iframe rect, hoisting the controls, skipping the layout morph) shipped 2026-06-11 and was **reverted the same day by operator decision**: the original chrome, button placement, and morph animation matter more than Twitch's pause behavior. The resize-triggered pause is now prevented without touching any chrome (operator-approved 2026-06-11 after screenshot review): `freezeTwitchPlayerSize` in `src/viewer-stream.mjs` pins the live channel iframe at an internal 1280×720 and contain-fits it into the panel with a CSS transform driven by a ResizeObserver — transforms scale rendered output without resizing the iframe's window, so the embed never receives the resize that re-runs the check, and layout toggles/window resizes no longer pause a playing stream (verified across repeated toggles with the morph enabled). Side effects: Twitch's hover chrome hugs the 16:9 video band rather than the panel edges, and video quality is selected for a constant 1280×720. Autoplay remains blocked by the occluded load state (the viewer clicks play, which starts audio, and audible tabs survive backgrounding); hidden/covered pauses remain accepted Twitch behavior — do not reattempt auto-resume.
