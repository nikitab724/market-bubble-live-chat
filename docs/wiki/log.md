...
## [2026-06-11] fix | Twitch embed stayed paused after tab/process switches — interactive player API + auto-resume

- Operator confirmed (incognito, post-deploy) that the Twitch embed still paused on every tab switch and window cover. Same embedded-experiences enforcement as the occlusion fix, triggered by the page being hidden instead of a resize; the embed pauses itself and never resumes. A spoofed `visibilitychange` alone did not reproduce the pause (Twitch's trigger sits deeper — frame production stopping), and Playwright cannot truly background a tab (`visibilityState` stays "visible" even via same-window tabs and CDP window-minimize), so the pause itself is only reproducible by a human; the resume path is what the page can own.
- Fix: live Twitch channels render through the interactive player API (`player.twitch.tv/js/embed/v1.js` → `Twitch.Player` in a `.twitch-player-host` div) instead of a bare iframe, with the bare iframe kept as fallback when the script cannot load. New `attachTwitchAutoResume` tracks PAUSE events that occur while `document.visibilityState === "hidden"` and calls `player.play()` when the page becomes visible; pauses made while visible (the viewer's own) are never overridden, and the listener detaches once the player leaves the DOM.
- Touched: `src/viewer-stream.mjs`, `styles.css` (`.twitch-player-host` sizing), `tests/viewer-stream.test.mjs` (4 auto-resume behavior tests), `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: TDD on `attachTwitchAutoResume` (resume after hidden pause, respect visible pause, no double-resume, listener cleanup) watched fail then pass; end-to-end drill against a live channel with the real player — hidden+pause → visible → playback resumed via the API, user-pause control stayed paused; layout toggles and muted autoplay re-verified clean; `npm test` 217/217. Real tab-switch behavior needs operator confirmation in a human browser.
