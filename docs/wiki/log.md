...
## [2026-06-11] feature | Admin "Clear chat history" empties the stored replay log

- Operator follow-up to the Kick leak incident: foreign messages already stored in the replay log kept replaying to new viewers until the 2h retention aged them out. `DELETE /api/admin/chat-events` (admin session required when a password is set) calls `clearEvents()` on the SSE hub, which delegates to a new `clear()` on both chat event stores. Ids stay monotonic across a clear (SQLite AUTOINCREMENT keeps allocating; the memory store keeps its counter), so a browser reconnecting with a pre-clear `Last-Event-ID` misses nothing sent afterwards. Connected browsers keep their in-memory feed until reload; new connections replay nothing.
- Admin UI: a "Clear chat history" details panel under `/admin/` (matching the X Bridge token / Change password pattern — opening the panel is the confirm step), one button, status line "Chat history cleared."
- Touched: `src/chat-event-store.mjs`, `src/chat-events.mjs`, `server.mjs`, `admin/index.html`, `admin/admin.mjs`, `styles.css`, `tests/chat-event-store.test.mjs`, `tests/chat-events.test.mjs`, `tests/server-contract.test.mjs`, `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: three new tests red→green (sqlite clear keeps ids monotonic; hub clear gives fresh connections an empty replay without resetting ids; contract: 401 logged out, 204 + store cleared with a session); `npm test` 195/195; `npm run build`; preview on the restarted dev server — `DELETE /api/admin/chat-events` returns 401 logged out (old code 405'd), the panel paints styled inside the editor (screenshot + elementFromPoint hit-test), no console errors.

## [2026-06-11] bugfix | X broadcast id survives stale admin saves; the bridge stops failing silently

- Incident: operator pasted the bridge token, popup said "Watching chat", but the admin X row stayed on the go-live prompt with 0 viewers. Diagnosis from prod `/api/live-state` (`x: no_sources`): no enabled X source had a `broadcastId`, so the server-side connector had nothing to attach to — the extension's capture report never landed or never survived. Three stacked defects: (1) `PUT /api/admin/sources` wrote the admin client's echoed `broadcastId`, so a save from an admin page loaded before the capture wiped the stored id (reproduced in an isolated lab: POST `/api/x-broadcast` → Save Profiles → id gone); (2) the extension never checked its `fetch` responses — 401 (wrong/per-other-backend token; local and prod now both have passwords and therefore different tokens) and 404 (handle mismatch) logged "reported broadcast" and looked like success, and the popup's "Watching chat" only reflects the DOM observer; (3) on `/i/broadcasts/<id>` URLs the 1s SPA poll reset `currentSourceHandle` to null, clobbering the popup selection for the next stream.
- Treatment: server-side, `carryXBroadcastIds` (replacing `dropStaleXBroadcastIds` in the save chain) restores each X source's stored `broadcastId` from previous config and ignores the client echo — same handle keeps the id, changed handle still drops it, clients cannot invent one. Extension-side, `reportBroadcastId()`/`sendMessage()` check `response.ok`, dedupe only on success, and feed a `bridge` state ("linked" / "unauthorized" / "no-source" / "no-broadcast-url" / "no-source-selected" / "error") to a new second status line in the popup with one-line actionable text; Apply/Retry no longer close the popup, so the verdict is visible; the SPA poll keeps the selected handle when the URL has none; manifest bumped to 1.1 so a reload is verifiable.
- Also investigated: "embed is misconfigured" on stream change — verified the exact wording comes from a Twitch player iframe with no `parent` param (lab repro); every current main-surface embed sets `parent` (and Twitch accepts `127.0.0.1`; parent mismatch renders a blank frame, empty channel says "Video playback error"), prod and local players render fine, so the remaining suspects are a stale cached bundle in an open tab or a referrer-stripping browser/extension. No code change.
- Touched: `server.mjs`, `extension/content.js`, `extension/popup.js`, `extension/popup.html`, `extension/manifest.json`, `tests/server-contract.test.mjs`, `docs/x-live-setup.md`, `docs/connectors.md`, `docs/wiki/log.md`.
- Verification: new stale-save contract test red→green (stored id kept when the PUT omits it; fabricated id dropped); `npm test` 196/196; `npm run build`; lab E2E on the fixed server — admin page loaded, id captured via POST, Save Profiles from the stale page, id still in `sources.json` and connector stays attached. Reminder: unpacked extension must be reloaded in `chrome://extensions` (now shows v1.1).

## [2026-06-11] ops | One-command local/self-host via Docker Compose

- Added `docker-compose.yml` at the repo root: a single `app` service that builds from the existing `Dockerfile`, publishes `4178:4178`, mounts the named volume `mb-data` at `/app/data` (SQLite chat DB, `sources.json`, `admin-password.json`), and uses `env_file` with `required: false` so `docker compose up -d --build` boots with zero config (open/local mode) and picks up providers when a `.env` exists. Mirrors the production container shape without the Firecrawl SSH deploy script, which is unchanged and remains the production path.
- Tightened `.dockerignore` to exclude `data/*.sqlite`, `data/*.sqlite-*`, and `data/admin-password.json` so `COPY . .` no longer bakes the local ~11 MB dev `chat-events.sqlite` or the dev admin hash into the image; a fresh named volume seeds with only `sources.json`.
- Touched: `docker-compose.yml` (new), `.dockerignore`, `README.md`, `docs/deployment.md`, `docs/wiki/index.md`, `docs/wiki/log.md`.
- Verification: `docker compose up -d --build` on Docker Desktop (Compose v2.32.4) — build context 1.91 MB (stale DB excluded), container Up on `0.0.0.0:4178`, `GET /api/public-config` → 200 with seeded sources, volume held a fresh 4 KB `chat-events.sqlite` (+ WAL/SHM) and `sources.json` with no `admin-password.json`; the DB kept its original create time across a full `compose down`/`up` (volume persistence) and `newproject3_mb-data` survived the final `down`. Torn down after verifying (no container left holding 4178).

## [2026-06-11] ops | Compose review: loopback default, no fixed container name, README leads with compose

- Review pass over the same-day compose work, aimed at "works on any server, super easy". Two fixes: (1) `ports` now interpolates `"${HOST_BIND:-127.0.0.1}:${HOST_PORT:-4178}:4178"` — the old `4178:4178` published on `0.0.0.0`, which on a fresh public server exposes the app while admin is still open (no password configured until first set); loopback matches the production tunnel-fronted shape. (2) Dropped `container_name: market-bubble-live`, which would name-conflict with the deploy-script container if compose ever ran on the Firecrawl box; compose-scoped commands never needed it.
- README `Run` now leads with the one-command compose path and demotes npm to a "Local development (npm)" subsection. `docs/deployment.md` compose section gains a fresh-server quickstart, the HOST_BIND/HOST_PORT overrides, the Compose v2.24+ floor (optional `env_file` long syntax), a note that `NODE_ENV=production` 404s the dev Kick injector in the container (verified via `enableDevRoutes` in `server.mjs`), and an explicit "the Firecrawl box keeps the push-to-main auto-deploy; don't run compose beside it" answer (port 4178 clash; legacy data is the `/opt/market-bubble-live/data` bind mount, not the compose volume).
- Touched: `docker-compose.yml`, `README.md`, `docs/deployment.md`, `docs/wiki/log.md`.
- Verification (cached image, no rebuild): default up → `docker compose ps` shows `127.0.0.1:4178->4178`, `GET /api/public-config` → 200, and the existing `newproject3_mb-data` volume re-attached (DB kept its 19:39:52 create time); `HOST_PORT=4179 docker compose up -d` → `127.0.0.1:4179->4178`, 200 on :4179; final `down` left the volume in place and port 4178 free.

## [2026-06-11] feature | Open pages follow admin saves without a reload

- Operator ask: saving in admin (removing/changing a connection, moving Show stream) required manually refreshing the main page; the player also never re-rendered on a selection change because `updateStreamPresence` only reacts to live↔offline flips. Design spec: `docs/superpowers/specs/2026-06-11-live-config-refresh-design.md`.
- Treatment: `GET /api/public-config` now includes `configVersion` (sha256 of the public projection, 12 hex chars — server-only fields like broadcast ids never bump it) and `PUT /api/admin/sources` broadcasts `config {version}` on the SSE hub after a successful save. `src/chat-runtime.mjs`: `loadPublicConfig` returns `{ sources, configVersion }` and `startBackendChatEvents` takes an optional `onConfigEvent`. `src/app.mjs` re-fetches config on a new version and applies it in place (`applySources`): chips/filters rebuild, per-source caches prune/warm (handle or profile re-points re-fetch emotes/badges/X profiles), live overlay survives, and the player re-renders only when `getStreamSelectionKey` (new in `src/viewer-stream.mjs`) changes. Replayed config events after SSE reconnects dedupe by version. Admin page unchanged (already adopts the save response); v2 untouched.
- Touched: `server.mjs`, `src/chat-runtime.mjs`, `src/viewer-stream.mjs`, `src/app.mjs`, `tests/server-contract.test.mjs`, `tests/chat-runtime.test.mjs`, `tests/viewer-stream.test.mjs` (new), `docs/architecture.md`, `docs/superpowers/specs/2026-06-11-live-config-refresh-design.md` (new), `docs/wiki/log.md`.
- Verification: 9 new tests red→green (server contract: version bump + one config event per save, stable version on identical save; chat-runtime: config shape, fallback copies, config listener; viewer-stream: selection fallback + key identity/changes/empty); `npm test` 205/205; `npm run build`; lab E2E via Playwright on an isolated server — with `/` open, a PUT moving Show stream to the X source swapped the live player Twitch→X within ~2s with no reload, a second PUT removing the Z source dropped its chip and returned the player to Twitch; console clean apart from Twitch-iframe-internal offline noise.

## [2026-06-11] diagnosis | Kick embed "misconfigured" = Cloudflare bot scoring on Kick's playback API; referrer/allow/UA-spoofing all reach the same error screen

- Operator kept seeing Kick's "This embed seems to be misconfigured" on the main page, in normal and incognito windows, eventually with a desktop UA too. Their console showed the real failure: `kick.com/api/v2/channels/<slug>/playback-url` answered **without** `Access-Control-Allow-Origin`, the player's fetch died as a CORS error ("Playback URL not found"), and Kick's generic error boundary rendered. In the same minute, a fresh Playwright browser got the same endpoint with `access-control-allow-origin: https://player.kick.com` and the stream played; curl from the dev machine got 403 `{"error":"Request blocked by security policy"}`. Conclusion: Kick's API responses are gated by Cloudflare bot scoring per client — flagged sessions lose the CORS header and the embed dies. The operator's session carried `cf_clearance` (had been challenged) after a day of rapid embed testing including DevTools device presets, whose spoofed Android UA over a desktop fingerprint is a classic bot signal and reproduces the block reliably.
- Verified secondary triggers for the same error screen on any client: missing `Referer` header (also breaks Twitch: "Whoops! This embed is misconfigured."), and missing `allow="autoplay; fullscreen"` delegation (site nested in a plain iframe). `autoplay=false` and unknown query params are harmless. Twitch plays under mobile UAs with a valid `parent`.
- Operator recovery: clear kick.com cookies, load kick.com once directly to re-pass the challenge, switch network if the IP is flagged; test mobile layout with Responsive dimensions instead of device presets. No app code change — the embed requests were correctly formed at every layer.
- Touched: `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: operator-supplied request headers and console trace; simultaneous fresh-browser play with captured `playback-url` response headers; curl 403 reproduction; referrer/allow/nested-frame lab matrices on prod against the live `eslcs` channel.
- Follow-up measurement after the operator reported flapping ("works after a couple refreshes, breaks again later"): 6 brand-new clean Playwright contexts loading the same live embed split 3 plays / 3 misconfigured, and a same-context triple reload went 0/3. So the dominant factor is **intermittent Kick-side blocking affecting all clients**, not operator-specific flagging; single-trial trigger attributions (mobile UA, missing allow) are correspondingly uncertain, while the no-referrer trigger retains concurrent passing controls. Architecture doc updated to the corrected framing.

## [2026-06-11] fix | X broadcast reports are ownership-checked so two live X sources cannot collapse onto one stream

- Live incident: with Ansem (`@blknoiz06`) and MB (`@marketbubble`) both live and configured as X sources, connecting the second stream made both sources show one stream's chat and viewer count, and reconnecting the first swapped both to it. Root cause: `extension/content.js` re-reports the page's broadcast id on every SPA URL change under the tab's previously selected source (`/i/broadcasts/<id>` URLs carry no handle), so browsing from one watched live page to the other posted the new broadcast id under the stale handle; `/api/x-broadcast` stored it, pointing both sources at the same broadcast. Both connector sockets then carried identical frames — shared occupancy, and one visible label per message because frame-uuid message ids dedupe client-side.
- Fix: `applyXBroadcastId` now takes the broadcast's owner handle and rejects a report with 409 when the owner is a different account than the source handle; `server.mjs` resolves the owner per report via new `xApiClient.getBroadcastOwner()` (guest token + `broadcasts/show.json`, `twitter_username` falling back to `username`). Lookup failures resolve to "" and are accepted, so X API flakiness cannot block auto-capture — only a confirmed mismatch rejects. The extension surfaces the 409 as a sticky "wrong-owner" verdict ("This broadcast belongs to a different account — reselect the source"); extension version 1.1 → 1.2.
- Touched: `src/x-api.mjs`, `server.mjs`, `extension/content.js`, `extension/popup.js`, `extension/manifest.json`, `tests/x-api.test.mjs`, `tests/server-contract.test.mjs`, `docs/x-live-setup.md`, `docs/connectors.md`, `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: TDD — new x-api owner-lookup tests and a server-contract test reproducing the incident (two X sources, cross-report rejected with 409 and config untouched, owning-source report stored, lookup-failure report accepted) watched fail then pass; full `npm test` 212/212. Pre-existing x-broadcast tests now inject a stub `xApiClient` so the suite stays off the network (the default client's real lookup actually 409'd one test, incidentally confirming the check works against live X).

## [2026-06-11] fix | Stream paused on every fullscreen/minimize layout toggle — Twitch embed occlusion ("style visibility") rule

- Operator report: a playing Twitch stream on `/` paused whenever the mini/full layout toggle ran, in both directions. Reproduced locally against a live channel by reading `video.paused` inside the player frame: play → toggle → paused (and a plain browser-window resize paused it too). Twitch's own console message named the cause: "minimum requirements for autoplay were not met: style visibility" — the embed runs an occlusion-aware visibility check, re-evaluated on every player resize. The floating social pill and the corner layout-toggle bracket painted over the iframe, so the check failed permanently: autoplay was refused at load, and any resize paused live playback. Controls-hidden A/B runs confirmed each overlay independently trips it, and with overlays clear the view-transition morph alone still paused mini→full (its snapshot overlay covers the live player mid-resize).
- Fix, all on the main surface: full layout insets `.stream-player` 68px under a band of the card's own `#080808` (pill and bracket float over the band, card looks unchanged); mini hoists the toggle bracket above the panel edge (`bottom: calc(100% + 12px)`, 42px) matching the pill's existing treatment; `ViewerApp.jsx` skips `startViewTransition` while a provider iframe is mounted (instant switch, like the existing Firefox path). Side effect gained: Twitch now autoplays at load (the same check had been blocking it) and window resizes no longer pause playback.
- Touched: `styles.css`, `src/ui/ViewerApp.jsx`, `tests/chat-interaction-contract.test.mjs` (geometry contracts updated + new clear-player contract), `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: Playwright drive of the real button against a live channel — autoplay at load, four consecutive toggles and a window resize with `video.currentTime` advancing throughout (2.9s → 14.4s, never paused); full/mini screenshots for design sanity; `npm test` 213/213.

## [2026-06-11] revert + fix | Auto-resume reverted; Twitch embed is click-to-play so audible playback survives tab/app switches

- Operator-confirmed on the live site: the interactive-player auto-resume did NOT recover from macOS process switches — Twitch pauses the muted player without any visibility signal reaching the page first, so the visibility-keyed resume never armed. `git revert db99292` restored the bare iframe embed (no embed script, no resume layer).
- The "it worked in the background before" behavior the operator remembers was the unmuted state: pre-occlusion-fix, broken autoplay forced a manual play click, which started audio, and audible tabs are never paused. With occlusion fixed, `autoplay=true` would autoplay muted and recreate the background-pause trap — so the live Twitch embed now sets `autoplay=false` on purpose. Load shows the play button, one click starts playback with sound, and background playback behaves as it always did. Contract test pins `autoplay=false` so allowed-autoplay does not creep back.
- Key institutional fact: Twitch's hidden/covered pause can fire with `document.visibilityState` still "visible" (Chrome occlusion tracking acts before/without the page-visible API flipping), so no page-side visibility hook can reliably resume it. Do not reattempt visibility-keyed resumes; the audible-tab path is the dependable one.
- Touched: revert of `src/viewer-stream.mjs`/`styles.css`/`tests/viewer-stream.test.mjs` (+docs) from db99292, then `src/viewer-stream.mjs` (`autoplay=false` + rationale comment), `tests/viewer-stream.test.mjs` (autoplay contract), `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: `npm test` 214/214; Playwright against a live channel — load shows unmuted paused player with play button, click starts unmuted playback, two layout toggles keep playing. Background-with-sound behavior is the operator's own long-lived experience plus the browser's audible-tab rule, not re-claimed from automation.

## [2026-06-11] revert | Full restore of original stream chrome, animations, and autoplay by operator decision

- Operator rejected the Twitch-pause workarounds end to end: process switches still paused the embed in their real environment, the mini-layout toggle bracket sat above the panel border instead of on it, and the layout morph animation was gone whenever an embed was mounted. Instruction was to restore everything to how it was.
- `styles.css`, `src/ui/ViewerApp.jsx`, `src/viewer-stream.mjs`, `tests/chat-interaction-contract.test.mjs`, and `tests/viewer-stream.test.mjs` are restored byte-for-byte to pre-fix state (5b397d4): social pill and toggle bracket back over the player, mini bracket back at 14/14 on the border, view-transition morph unconditional again, `autoplay=true` back on the channel embed. The X broadcast ownership fix (unrelated, confirmed working) stays.
- Accepted consequences, documented in `docs/architecture.md`: Twitch's "style visibility" rule blocks autoplay (viewer clicks play, which starts audio) and pauses the stream on layout toggles/window resizes; hidden/covered pauses can fire with no visibility signal reaching the page, so no page-side auto-resume is reliable. Do not reattempt any of these workarounds without operator sign-off.
- Touched: the five restored files plus `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: `git diff 5b397d4` over the five files is empty; `npm test` green; build clean.

## [2026-06-11] fix | Layout toggle no longer pauses Twitch — frozen iframe size + transform scaling; chat column trimmed for a larger full-layout video

- Operator-approved replacement for the reverted chrome workarounds, with all chrome untouched (pill, bracket, morph, autoplay). Twitch only re-runs the embedded-experiences check (the pause trigger) when its iframe resizes, so `freezeTwitchPlayerSize` (`src/viewer-stream.mjs`) pins the live channel iframe at an internal 1280×720 and contain-fits it into the panel via `translate(...) scale(...)` updated by a ResizeObserver. Transforms never resize the iframe's window, so toggles and window resizes no longer reach the player as resizes. 16:9 base keeps the video band where the fluid iframe put it in both layouts (Twitch letterboxes to 16:9 internally; our #080808 bars replace its internal ones). Known cosmetic delta: Twitch's hover chrome hugs the video band instead of the panel edges in full layout.
- Full-layout video enlarged per operator request: `.viewer-shell` chat column `minmax(360px,420px)` → `minmax(340px,384px)`; the stream is width-bound, so the band gains ~36×20px at any viewport.
- Touched: `src/viewer-stream.mjs`, `styles.css`, `tests/chat-interaction-contract.test.mjs` (new freeze/width contract; updated pinned column width), `docs/architecture.md`, `docs/wiki/log.md`.
- Verification: live channel, overlays in original positions, morph enabled — seven layout toggles plus window resizes with `video.currentTime` advancing throughout and the iframe's inner size locked at 1280×720; both-layout screenshots reviewed and approved by the operator; `npm test` 213/213. Backgrounding pause unchanged (accepted; audible tabs survive it).
